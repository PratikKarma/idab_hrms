...
class Utility extends Model
{
    ...
    public static function employeePayslipDetail($employeeId, $month)
    {
        ...
        // leave
        $salary = $employess->salary;
        $per_day_amount = $salary / Carbon::now()->daysInMonth;

        $today = Carbon::now();
        $days_so_far = $today->day;

        $attendance_count = AttendanceEmployee::where('employee_id', $employeeId)
        ...
        $deduction['leave'][] = (object) array(
            ...
            'total_leave_days' => $absent_days . ' days',
        ...
        $payslip['totalDeduction'] = $totalloan + $totaldeduction + $totalPansion + $leave_deduction;

        $payslip['net_salary'] = $salary + $payslip['totalEarning'] - $payslip['totalDeduction'];
