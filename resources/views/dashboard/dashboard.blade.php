                                ...
                                @if ($canClockIn)
                                    @if ($clockInLate || $clockInEarly)
                                        ...
                                        @if (!$requestStatus)
                                            ...
                                            <button type="button" class="btn btn-warning clock-action" data-action="clock_in"
                            ...
                            <div class="col-md-4 float-left">
                                @if (!empty($employeeAttendance) && $employeeAttendance->clock_out == '00:00:00')
                                    ...
                                    @if (empty($breaks) || (end($breaks)['end'] ?? null) !== null)
                                        <button type="submit" value="1" name="break" id="break_start"
                                            class="btn btn-warning">{{ __('START BREAK') }}</button>
                                    @else
                                        <button type="submit" value="1" name="break" id="break_end"
                                            class="btn btn-success">{{ __('END BREAK') }}</button>
                                    @endif
                                    {{ Form::close() }}
                                    ...
                                    @if (!$inAllowedWindow)
                                        ...
                                        @if (isset($attendanceRequest) && $attendanceRequest->type == 'clock_in')
                                            ...
                                            @if (is_null($attendanceRequest->clock_out))
                                                ...
                                                <button type="button" class="btn btn-warning clock-action" data-action="clock_out"
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const routes = {
            ...
            @if(!empty($attendanceRequest) && $attendanceRequest->id)
                attendanceRequestClockOut: "{{ route('attendancerequests.clock_out', $attendanceRequest->id ?? 0) }}",
            @endif
        @if(!empty($employeeAttendance) && $employeeAttendance->id)
            employeeAttendanceUpdate: "{{ route('attendanceemployee.update', $employeeAttendance->id ?? 0) }}",
        @endif
    };
    ...
    document.querySelectorAll('.clock-action').forEach(btn => {
        btn.addEventListener('click', function (e) {
            e.preventDefault();
            ...
            if (action === 'clock_out') {
                ...
                console.log('Current Time (minutes):', nowMin);
                console.log('Office End (minutes):', endMin);
                console.log('Early Threshold:', earlyThreshold);
                console.log('Late Threshold:', lateThreshold);
                console.log('Is Too Early?', nowMin < earlyThreshold);
                console.log('Is Too Late?', nowMin > lateThreshold);
                console.log('In Allowed Window?', nowMin >= earlyThreshold && nowMin <= lateThreshold);

                // Check if outside allowed window
                if (nowMin < earlyThreshold || nowMin > lateThreshold) {
                    // Outside window - need request (button should be orange)
                    openReasonModal('clock_out');
                    return;
                }

                // Within allowed time - normal clock out (button should be red)
