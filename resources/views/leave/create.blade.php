        ...
        <div class="col-md-12">
            <div class="form-group">
                ...
                @can('Create Leave Type')
                    <div class="text-xs mt-1">
                        {{ __('Create leave type.') }} <a href="{{ route('leavetype.index') }}"><b>{{ __('Click here') }}</b></a>
                    </div>
                @endcan
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                ...
                {{ Form::date('start_date', null, ['class' => 'form-control d_week', 'required' => 'required', 'autocomplete' => 'off']) }}
            </div>
        </div>
        <div class="col-md-6">
            <div class="form-group">
                {{ Form::label('end_date', __('End Date'), ['class' => 'col-form-label']) }}<x-required></x-required>
                {{ Form::date('end_date', null, ['class' => 'form-control d_week current_date', 'required' => 'required', 'autocomplete' => 'off']) }}
            </div>
        </div>
    </div>
    <div class="row">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="form-group">
                ...
                @if ($plan->enable_chatgpt == 'on')
                    <a href="javascript:void(0)" data-size="md" class="btn btn-primary btn-icon btn-sm" data-ajax-popup-over="true"
