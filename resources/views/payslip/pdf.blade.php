...
<div class="modal-body">
    <div class="text-md-end mb-2">
        <a href="javascript:void(0)" class="btn btn-sm btn-primary me-1" data-toggle="tooltip" data-bs-placement="top"
            title="{{ __('Download') }}" onclick="saveAsPDF()"><span class="fa fa-download"></span></a>

        @if (\Auth::user()->type == 'company' || \Auth::user()->type == 'hr')
            <a href="{{ route('payslip.send', [$employee->id, $payslip->salary_month]) }}" class="btn btn-sm btn-warning"
                data-toggle="tooltip" data-bs-placement="top" title="{{ __('Mail Send') }}"><span
                            ...
                            <div class="table-responsive">
                                <table class="table  table-md">
                                    <tbody>
                                        ...
                                        @foreach ($payslipDetail['earning']['pearks'] as $pearks)
                                            <tr>
                                                <td>{{ __('Pearks') }}</td>
                                                <td>{{ $pearks->title }}</td>
                                                <td>Coupon ({{ $pearks->peark_coupon }})</td>
                                                <td class="text-right">
                                                    {{ \Auth::user()->priceFormat($pearks->amount) }}
                                                </td>
                                            </tr>
                                        @endforeach
